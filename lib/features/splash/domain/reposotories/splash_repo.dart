...
class SplashRepo {
  ...
  bool showIntro() {
    return false; //sharedPreferences!.getBool(AppConstants.onBoardingSkip)?? true;
  }
  ...
  // pop-up
  Future<ApiResponseModel> getpopupImage() async {
    try {
      final response = await dioClient!.get(AppConstants.popupUrl);
      return ApiResponseModel.withSuccess(response);
    } catch (e) {
      return ApiResponseModel.withError(ApiErrorHandler.getMessage(e));
    }

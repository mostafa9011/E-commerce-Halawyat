...
class CategoryProvider extends ChangeNotifier {
  ...
  List<Product>? categoryAllProductList;
  ...
  void getCategoryProductList(String categoryID) async {
    ...
    if (apiResponse.response != null &&
      ...
      apiResponse.response!.data.forEach((category) =>
          categoryAllProductList!.add(Product.fromJson(category)));
...
//// parents
  Future<ApiResponseModel> getParentList(BuildContext context, bool reload,
      {int? id}) async {
    ApiResponseModel apiResponse = await categoryRepo.getParentList();

    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      parentList = [];
      apiResponse.response!.data.forEach(
        (category) => parentList!.add(
